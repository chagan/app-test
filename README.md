...
```
cd app-test
mkvirtualenv --no-site-packages app-test
...
```
* * * * * ubuntu bash /home/ubuntu/apps/app_test/repository/cron.sh fab $DEPLOYMENT_TARGET cron_jobs.test 
```
...
|--------|------|-----|-----|--------|--------|
|app-test|tweet|`location`||||
|app-test|facebook|`location`||||
|app-test|email|`location`||||
|app-test|new-comment||||
|app-test|open-share-discuss||||
|app-test|close-share-discuss||||
|app-test|summary-copied||||
|app-test|featured-tweet-action|`action`||``tweet_url``|
|app-test|featured-facebook-action|`action`||``post_url``|
